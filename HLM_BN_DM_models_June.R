...
data_full <- read.csv('C:\\Users\\karthik\\Google Drive\\GSA DATA\\Data integration\\working_files\\all_data_june16_trim_variables.csv')
...
data_input <- read.csv('C:\\Users\\karthik\\Google Drive\\GSA DATA\\Data integration\\working_files\\all_data_june17_g.csv')
...
#data_conservative <- na.omit(data_input)
...
for (i in 1: nrow(data_mod) )
{
  ...
  else { data_mod$Temperature_feel[i] <- "z"}
}
...
data_con <- data.frame(Pressure = data_in$Pressure, CO2 = data_in$CO2, Activity = data_in$Activity, Window_dist = data_in$Window_dist)
...
data_in$wing <- as.factor(data_in$wing)
